...
function showQueston() {
  ...
  for (var i = 0; i < questions[indexNum].options.length; i++) {
    ansParent.innerHTML += `<div class = "col-md-6 py-2">
        <button onclick = "checkAns('${questions[indexNum].options[i]}','${questions[indexNum].answer}')" class = "btn btn-primary px-5 rounded-pill w-100">
...
function checkAns(a, b) {
  ...
  if (indexNum+1 == questions.length) {
    main.innerHTML= `<h1 class='text-center text-white'> Your Marks are : ${marks} </h1>`;
  } else {
    nextQuestion();
  }
}
